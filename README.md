...
| 🏆 **Wall of Fame** | `≥ GLORY_KILLS` kills, an ace (15+ kills), or a **carry**: top-fragger of the lobby, best impact on his team with ≥ `FAME_CARRY_SHARE`% of its output, `≥ FAME_CARRY_KILLS` kills and K/D ≥ `FAME_CARRY_KD` or ADR ≥ `FAME_CARRY_ADR` (`GLORY_POSTS_ENABLED`) | gold card: fame awards (👑 Top of the lobby · 🚂 Hard carry / 🥀 Wasted · 🃏 Ace · 🛡️ Untouchable · ⭐ MVP machine · 🎯 Headhunter · 🧊 Clutch king · 🚀 Entry king · 🧪 Utility master), praise line with award-driven follow-ups, and a **carry report** strip (share of the team's output, kill/damage/MVP share, rank, facts) |
